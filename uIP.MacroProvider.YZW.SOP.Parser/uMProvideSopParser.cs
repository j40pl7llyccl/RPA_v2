    ...
    public class uMProvideSopParser : UMacroMethodProviderPlugin
    {
        ...
        private bool ParseVideoSplit(out UDataCarrier ret, params UDataCarrier[] ctx)
        {
            ...
            foreach (var kv in sd.Data)
            {
                ...
                if (!float.TryParse(kv.Key.Replace("s", ""), out float offsetSec))
                    continue;

                // ✅ 只處理有值的 key
                if (kv.Values == null || string.IsNullOrWhiteSpace(kv.Values[0]))
                    continue;

                var fullPath = Path.Combine(dir, kv.Values[0].Trim());
                got[fullPath] = currT.AddSeconds(offsetSec);
                //var offsetSec = float.Parse( kv.Key.Replace( "s", "" ) );
                //got.Add( Path.Combine( dir, kv.Values[ 0 ].Trim() ), currT.AddSeconds( offsetSec ) );
            }

            UDataCarrier.Set(dic, filePathKey, got.Keys.ToArray());
