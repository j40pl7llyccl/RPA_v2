...
namespace uIP.MacroProvider.StreamIO.DividedData
{
    ...
    public class FileDistributor : UMacroMethodProviderPlugin
    {
        ...
        // 生成各子資料夾的 txt 檔，列出所有圖片檔案路徑
        private void CreateTxtFiles(string folderPath)
        {
        ...
        /// <summary>
        /// 供外部 WinForm 或 Macro 直接呼叫的方法：進行檔案搬移，
        /// 並於各子資料夾內產生列出圖檔路徑的 txt 檔。
        /// </summary>
        /// <param name="showMessage">若為 true 則在 UI 環境下顯示完成訊息</param>
        public int StartConvertByCode(string folderPath, double trainRatio, double testRatio, double valRatio, bool showMessage = false)
        {
            int moved = DistributeFiles(folderPath, trainRatio, testRatio, valRatio);
            CreateTxtFiles(folderPath);

            if (showMessage)
            {
                MessageBox.Show(
                    $"StartConvertByCode 已完成\n" +
                    $"搬移數量 = {moved}\n" +
                    $"(來源路徑: {folderPath})\n" +
