...
namespace uIP.MacroProvider.StreamIO.DividedData
{
    ...
    /// 透過 Macro 模式或 WinForm 直接呼叫執行三條路徑的搬檔作業，
    /// 並根據各路徑勾選的 Train/Test/Val 選項搬移到目標資料夾中。
    /// 核心轉檔邏輯均封裝於本類別中，並在 Initialize 方法中註冊 Macro 方法。
    /// </summary>
    public class ProcessPath : UMacroMethodProviderPlugin
    {
        ...
        {
            // 可在此針對 MacroInstance 加入初始化檢查或預設參數設定
        ...
        // Macro 模式下的執行邏輯：從 MacroInstance 讀取 12 格參數後依序呼叫轉檔邏輯
            ...
            ref fpUDataCarrierSetResHandler ResultCarrierHandler)
        {
            try
            {
                ...
                {
                    bStatusCode = false;
                    strStatusMessage = "ProcessPath: 需要 12 個參數 => 3 個路徑 + 9 個布林值.";
                    return null;
                }
                ...
                bStatusCode = true;
                strStatusMessage = $"ProcessPath 完成，總搬移數 = {totalMoved}";
            ...
            {
                bStatusCode = false;
                strStatusMessage = $"Macro_ProcessPath 發生例外：{ex}";
                return null;
            }
        }

        /// <summary>
        /// 供 UI 呼叫的公開方法，依據傳入的 3 個路徑與各選項，統一執行搬檔作業。
        /// 本方法與 Macro 模式共用同一核心搬檔邏輯 (MoveFiles)。
        /// </summary>
        public void StartConvertByCode(
        ...
        /// <summary>
        /// 實際的搬檔邏輯：給定來源路徑與對應的 Train/Test/Val 選項，
